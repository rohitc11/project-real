...
export default async function ServiceDetailPage({ params }: Params) {
  ...
  return (
    <>
      <PageHero
        sheet={`A-${service.index}`}
        eyebrow={`Service ${service.index}`}
        title={service.title}
        intro={service.summary}
      />

      <Section sheet={`A-${service.index}`} eyebrow="Detail">
            ...
            <ul className="mt-8 flex flex-col gap-4">
              {service.outcomes.map((item, index) => (
                <Reveal as="li" key={item} delay={index * 60}>
                  <div className="border border-line-soft bg-paper p-6 sm:p-7">
                    <p className="type-title text-lg text-ink text-pretty">
                      {item}
                    </p>
                  </div>
                </Reveal>
      ...
      <Section sheet="A-00" eyebrow="Schedule" heading="Other disciplines">
            ...
            <Reveal as="li" key={item.slug} delay={index * 40}>
              ...
              >
                <span className="flex items-baseline gap-5">
                  <span className="type-data text-xs text-muted">{item.index}</span>
                  <span className="type-title text-xl text-ink transition-colors duration-300 group-hover:text-accent-deep sm:text-2xl">
                    {item.title}
                  </span>
                </span>
                <ArrowRight className="shrink-0 text-muted transition-colors group-hover:text-accent" />
              </Link>
            </Reveal>
          ))}
        </ul>
